...
server <- function(input, output) {
  ...
  output$gg <- renderPlot({
    ...
    board %>%
      ...
      geom_point(size = isolate(input$letsize)+2.5, color = '#ffffcc', shape = 15) +
      geom_text(size = isolate(input$letsize), color = '#0c2c84') +
      coord_fixed(xlim = c(.5,i+.5), ylim = c(.5,j+.5)) +
      theme_minimal()
    #theme(panel.background = element_rect(fill = "#2c7fb8"))
